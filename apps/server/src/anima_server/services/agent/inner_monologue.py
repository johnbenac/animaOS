    ...
    try:
        with factory() as db:
            ...
            prompt = QUICK_REFLECTION_PROMPT.format(
                inner_state=inner_state_block.content if inner_state_block else "No state yet.",
                working_memory=working_memory_block.content if working_memory_block else "Empty.",
    ...
    *,
    ...
    try:
        with factory() as db:
            ...
            episodes_text = "\n".join(
                f"- {ep.date}: {ep.summary} (topics: {', '.join(ep.topics_json or [])})"
            ...
            prompt = DEEP_MONOLOGUE_PROMPT.format(
                ...
                growth_log=_last_n_entries(
                    blocks["growth_log"].content if "growth_log" in blocks else "", 5
                ),
                intentions=blocks["intentions"].content if "intentions" in blocks else "None.",
